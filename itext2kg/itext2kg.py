...
class iText2KG:
    ...
    def build_graph2(
        ...
        # Initialize global entities and relationships
        print("[INFO] ------- Extracting Entities from the Document 1")
        ...
        for i in range(1, len(sections)):
            print(f"[INFO] ------- Extracting Entities from the Document {i + 1}")
            ...
            print(f"[INFO] ------- Extracting Relations from the Document {i + 1}")
            relationships = self.irelations_extractor.extract_verify_and_correct_relations(
                context=sections[i], 
                entities=processed_entities, 
                rel_threshold=rel_threshold,
                max_tries=max_tries, 
            ...
            processed_relationships, _ = self.matcher.process_lists(
                list1=relationships, 
                list2=global_relationships, 
                threshold=rel_threshold
            )
            
            global_relationships.extend(processed_relationships)
        
        # If there's an existing knowledge graph, merge with it
        if existing_knowledge_graph:
            print(f"[INFO] ------- Matching the Document Entities and Relationships with Existing Global Entities/Relations")
        ...
        # Construct the final knowledge graph and remove duplicates
        constructed_kg = KnowledgeGraph(entities=global_entities, relationships=global_relationships)
        constructed_kg.remove_duplicates_entities()
        constructed_kg.remove_duplicates_relationships()
